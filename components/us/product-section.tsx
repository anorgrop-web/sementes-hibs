...
const KIT_COLOR_MAP: Record<string, { color: string; imageIndex: number }> = {
  "20-seeds": { color: "Mixed Colors", imageIndex: 0 },
...
export function ProductSection() {
  const [selectedColor, setSelectedColor] = useState("Mixed Colors")
  ...
  const getShopifyCartUrl = () => {
    if (selectedKit === "20-seeds") return null
  ...
  return (
    <div className="w-full bg-white">
      <UpsellModal isOpen={showUpsellModal} onClose={() => setShowUpsellModal(false)} onAccept={handleAcceptUpsell} onDecline={handleDeclineUpsell} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div className="flex h-full touch-pan-y">
                  {PRODUCT_IMAGES.map((image, index) => (
                    <div key={index} className="flex-[0_0_100%] min-w-0 h-full relative">
                      <Image src={image || "/placeholder.svg"} alt={`Product image ${index + 1}`} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" priority={index === 0} />
                    </div>
                  ))}
                </div>
              </div>
              <button onClick={prevSlide} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" aria-label="Previous image"><ChevronLeft className="w-6 h-6 text-gray-800" /></button>
              <button onClick={nextSlide} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" aria-label="Next image"><ChevronRight className="w-6 h-6 text-gray-800" /></button>
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                {PRODUCT_IMAGES.map((_, index) => (
                  <button key={index} onClick={() => goToSlide(index)} className={cn("w-2 h-2 rounded-full transition-all duration-300", currentSlide === index ? "bg-white w-6" : "bg-white/50 hover:bg-white/75")} aria-label={`Go to slide ${index + 1}`} />
                ))}
              </div>
            </div>
            <div className="hidden lg:flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              {PRODUCT_IMAGES.map((image, index) => (
                <button key={index} onClick={() => goToSlide(index)} className={cn("flex-shrink-0 w-20 h-20 rounded-md border-2 overflow-hidden transition-all", currentSlide === index ? "border-black" : "border-gray-200 hover:border-gray-300")}>
                  <Image src={image || "/placeholder.svg"} alt={`Thumbnail ${index + 1}`} width={80} height={80} sizes="80px" className="h-full w-full object-cover" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="bg-warning-yellow text-white text-xs font-bold px-2 py-1 rounded">SAVE {getDiscountPercentage()}%</span>
            </div>
            ...
            <div ref={offersRef} className="space-y-3">
              ...
              <div className="space-y-2">
                {kits.map((kit) => (
                  <button key={kit.id} onClick={() => handleKitSelect(kit.id)} className={cn("w-full px-4 py-3 rounded-full border text-sm font-medium text-left transition-all", selectedKit === kit.id ? "bg-black text-white border-black" : "bg-white text-black border-gray-300 hover:border-gray-400")}>{kit.label}</button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                <Button variant="outline" size="icon" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="h-10 w-10 rounded-md"><Minus className="h-4 w-4" /></Button>
                <span className="text-lg font-semibold w-12 text-center">{quantity}</span>
                <Button variant="outline" size="icon" onClick={() => setQuantity(quantity + 1)} className="h-10 w-10 rounded-md"><Plus className="h-4 w-4" /></Button>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between max-w-md mx-auto">
                <div className="flex flex-col items-center flex-1">
                  <div className="relative">
                    <div className="w-12 h-12 rounded-full bg-gray-900 flex items-center justify-center"><ShoppingCart className="w-6 h-6 text-white" /></div>
                    <div className="absolute top-6 left-12 w-20 sm:w-24 h-0.5 bg-gray-900" />
                  </div>
                  <div className="mt-3 text-center"><p className="text-sm font-bold">{formatDate(getUSDate(0))}</p><p className="text-xs text-gray-600">Ordered</p></div>
                </div>
                <div className="flex flex-col items-center flex-1">
                  <div className="relative">
                ...
                <div className="flex flex-col items-center flex-1">
                  ...
                  <div className="mt-3 text-center"><p className="text-sm font-bold">{formatDateRange(getUSDate(10), getUSDate(15))}</p><p className="text-xs text-gray-600">Delivered</p></div>
                </div>
              </div>
            </div>

            <div className="flex justify-center pt-4">
              <Image src="/images/design-mode/Pagamento%20seguro.webp" alt="Secure Payment Methods" width={448} height={40} sizes="(max-width: 448px) 100vw, 448px" className="max-w-full h-auto w-full max-w-md" />
            </div>
          </div>
        </div>
      </div>

      <div className={cn("fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg transition-transform duration-300 z-50", showStickyBar ? "translate-y-0" : "translate-y-full")}>
